<?php
	class WpFastestCacheRenderBlocking{
		...
		public function set_js(){
			...
			foreach ($script_tag as $key => $value) {
				...
				*/
				...
				if(preg_match("/static\.mailerlite\.com/i", $value["text"])){
					continue;
				}
				...
				*/
				...
				/*
				ConvertKit.com
				<script async data-uid="b3bb8ec8aa" src="https://thoughtful-artisan-9376.ck.page/b3bb8ec8aa/index.js"></script>
				*/
				if(preg_match("/\.ck\.page\/[^\/]+\/index\.js/i", $value["text"])){
					continue;
				}


				if(preg_match("/mediavine|social-pug/i", $value["text"])){
					continue;
				}
		...
		public function defer_load_inline_js($script){
			...
			if(preg_match("/rs-initialisation-scripts/i", $script)){
